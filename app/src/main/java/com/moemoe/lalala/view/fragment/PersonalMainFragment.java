...
import com.moemoe.lalala.view.activity.NewBagV2Activity;
...
//            R.drawable.shape_rect_label_tab_blue};
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        mTvNum.setText(getString(R.string.label_liuyanban, 0));
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.iv_level_name_details:
                if (entity != null) PersonalLevelActivity.startActivity(getContext(),
                        entity.getLevelName(), entity.getLevelColor(), entity.getScore(),
                        entity.getLevelScoreStart(), entity.getLevelScoreEnd(), entity.getLevel());
                break;
            ...
            case R.id.tv_more_add:
                if (isOpenBag) {
                    Intent i2 = new Intent(getContext(), NewBagV2Activity.class);
                    i2.putExtra("uuid", uuid);
                    if (uuid.equals(PreferenceUtils.getUUid())) {
                        i2.putExtra("type", "my");
                    } else {
                        i2.putExtra("type", "taren");
                    }
                    i2.putExtra("name", "书包");
                    startActivity(i2);
                } else {
    ...
    public void onLoadInfoSuccess(final PersonalMainEntity o) {
        ...
        if (!uuid.equals(PreferenceUtils.getUUid())) {
            ...
            if (TextUtils.isEmpty(entity.getPicPath())) {
                ...
                Glide.with(getContext())
                        ...
                        .placeholder(R.drawable.shape_gray_e8e8e8_background)
                        .into(mIvRole);
                if (PreferenceUtils.getUUid().equals(uuid)) {
        ...
        mTvNum.setText(getString(R.string.label_liuyanban, entity.getCommentCount()));
        ...
        if (entity.getFolderList().size() > 0) {
            ...
            for (int n = 0; n < entity.getFolderList().size(); n++) {
                ...
                if (item.getTexts().size() == 2) {
                    ...
                    TagUtils.setBackGround(item.getTexts().get(0), tag1);
                    TagUtils.setBackGround(item.getTexts().get(1), tag2);
                ...
                if (item.getType().equals("ZH")) {
                    ...
                    mark.setText("文章");
                    mark.setBackgroundResource(R.drawable.shape_rect_zonghe);
                } else if (FolderType.SP.toString().equals(item.getType())) {
                    mark.setText("视频集");
                    mark.setBackgroundResource(R.drawable.shape_rect_shipin);
                } else if (FolderType.YY.toString().equals(item.getType())) {
                    mark.setText("音乐集");
