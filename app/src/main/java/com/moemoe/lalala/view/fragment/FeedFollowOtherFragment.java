...
public class FeedFollowOtherFragment extends BaseFragment implements FeedFollowOtherContract.View {
    ...
    private void formerFunction(final FeedFollowType2Entity entity) {
        ...
        mItem2Fragment = FeedFollowOther2Fragment.newInstance(id, getArguments().getString("title"), true);
    ...
    public void feedFoollowOtherEvent(FeedFollowOtherEvent event) {
        if (id.equals(event.getId())) {
            if (event.getNum() != 0) {
                mTvFileNum.setText("内容 " + event.getNum());
            }
        }
    }
