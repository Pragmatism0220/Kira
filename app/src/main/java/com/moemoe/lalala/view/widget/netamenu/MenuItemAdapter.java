...
public class MenuItemAdapter extends RecyclerView.Adapter {
    ...
    public void onBindViewHolder(final RecyclerView.ViewHolder holder, final int position) {
        if(holder instanceof HorizontalViewHolder){
            ...
            boolean textColor = list.get(position).isTextColor();
            if (textColor){
                ((HorizontalViewHolder) holder).tvText.setTextColor(context.getResources().getColor(R.color.orange_ed853e));
            }else {
                ((HorizontalViewHolder) holder).tvText.setTextColor(context.getResources().getColor(R.color.gray_929292));
            }
