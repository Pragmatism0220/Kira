...
public class DocDetailPresenter implements DocDetailContract.Presenter {
    ...
    @Override
    public void requestTopComment(String id, int type, int sortType, final int index, final int sart) {
        if (type == 0) {//0 转发 1 评论
            apiService.loadRtComment(id, ApiService.LENGHT, index)
                    ...
                    .subscribe(new NetResultSubscriber<ArrayList<CommentV2Entity>>() {
                        ...
                        public void onSuccess(ArrayList<CommentV2Entity> commentV2Entities) {
                            if (view != null)
                                view.onLoadTopCommentSuccess(commentV2Entities, sart == 0);
                        }
    ...
    @Override
    public void loadDocTop(final boolean status, final String docId) {
        apiService.loadDocTop(!status, docId)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new NetSimpleResultSubscriber() {
                    @Override
                    public void onSuccess() {
                        if (view != null) view.onDocTopSuccess(!status, docId);
                    }
    ...
    @Override
    public void loadDocExcellent(final boolean status, final String docId) {
        apiService.loadDocxcellent(!status, docId)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
                .subscribe(new NetSimpleResultSubscriber() {
                    @Override
                    public void onSuccess() {
                        if (view != null) view.onDocExcellentSuccess(!status, docId);
                    }
    ...
    public void loadJoin(String tagId, final boolean join) {
        apiService.loadTagJoin(join, tagId)
                ...
                .subscribe(new NetSimpleResultSubscriber() {
                    @Override
                    public void onSuccess() {
                        if (view != null) view.onLoadJoinSuccess(!join);
                    }
